...
SELECT name
FROM world
WHERE gdp > ALL(SELECT gdp FROM world WHERE continent='Europe' AND population > 0);

--7. Find the largest country (by area) in each continent, show the continent, the name
-- and the area:
...
IN (SELECT MIN(name) FROM world GROUP BY continent);
